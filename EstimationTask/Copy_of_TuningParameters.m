...
tuningParams.q     = zeros(1, nNeurons) + 2;   % 1 or 2?? (variable) Set it to some constant. Controls the sharpness and amplitude of peak FR.
tuningParams.w     = zeros(1, nNeurons) + 0;   % 0 or 1?? Doesn't matter since untuned component is zero. Weight of untuned filter. Set it to 0. (fixed) Doesn't matter what is val is becz untuned filter amp is zero.
tuningParams.e1    = zeros(1, nNeurons);       % Stimulus independent spontaneous discharge (variable) Controls dynamic range.
tuningParams.e2    = zeros(1, nNeurons);       % Stimulus dependent spontaneous discharge (variable) Controls dynamic range.
tuningParams.gam   = zeros(1, nNeurons);       % Controls response amplitude (variable) Controls dynamic range.
tuningParams.beta  = zeros(1, nNeurons) + 0;   % Stimulus independent constant (variable) Controls dynamic range.
...
% form trial by trial
tuningParams.alpha(:)   = 1 + 5*rand(1, nNeurons);            % 1 + 5*rand(1, nNeurons);  Aspect ratio uniformly sampled from 0 - 5
tuningParams.b(:)       = randsample(values,nNeurons,true,p); % Derivative order - Just set it 2 (Zoey's paper). Non integer vaalues can give imaginary values. More like log uniform 0.0125 + 8*rand(1, nNeurons); 
tuningParams.q(:)       = exp( log(1.8)*rand(1, nNeurons) );  % Transduction (this is not uniformly distributed) exp( log(1.8)*rand(1, nNeurons) )
...
tuningParams.e1(:)        = lognrnd(0.8, 0.6, 1, nNeurons);       % Range: 0 - 10 ips, Might not be correct initilization
% tuningParams.e2(:)      = lognrnd(0.8, 0.6, 1, nNeurons);
tuningParams.gam(:)       = 2000;
